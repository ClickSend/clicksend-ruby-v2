...
module ClickSend
  class DefaultSenderDefaultSenderStrategiesInner < ApiModelBase
    # Type of the sender.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClickSend::DefaultSenderDefaultSenderStrategiesInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClickSend::DefaultSenderDefaultSenderStrategiesInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [sender_type, sender_id, sender_country_code, priority, status, note].hash
    end
