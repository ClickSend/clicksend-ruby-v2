...
module ClickSend
  class EmailDeliveryReceiptRule < ApiModelBase
    # The ID of the receipt rule.
    attr_accessor :receipt_rule_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClickSend::EmailDeliveryReceiptRule` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClickSend::EmailDeliveryReceiptRule`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'receipt_rule_id')
        self.receipt_rule_id = attributes[:'receipt_rule_id']
      end
      ...
      if attributes.key?(:'action_address')
    ...
    def ==(o)
      ...
      self.class == o.class &&
          receipt_rule_id == o.receipt_rule_id &&
    ...
    def hash
      [receipt_rule_id, rule_name, match_type, action, action_address, enabled].hash
    end
