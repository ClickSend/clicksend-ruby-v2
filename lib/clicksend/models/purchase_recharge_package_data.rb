...
module ClickSend
  class PurchaseRechargePackageData < ApiModelBase
    # The ID of the user.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClickSend::PurchaseRechargePackageData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClickSend::PurchaseRechargePackageData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'amount_aud')
        self.amount_aud = attributes[:'amount_aud']
      end
    ...
    def hash
      [user_id, amount, currency, amount_aud, date].hash
    end
