  ...
  class Recipient < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClickSend::Recipient` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClickSend::Recipient`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'duplex')
        self.duplex = attributes[:'duplex']
      end

      if attributes.key?(:'post_pages')
        self.post_pages = attributes[:'post_pages']
      end
      ...
      if attributes.key?(:'priority_post')
    ...
    def hash
      [user_id, subaccount_id, message_id, address_name, address_line_1, address_line_2, address_city, address_state, address_postal_code, address_country, return_address_id, custom_string, schedule, source, colour, duplex, post_pages, post_price, priority_post, date_added, status, _file_url, _return_address, _api_username].hash
    end
