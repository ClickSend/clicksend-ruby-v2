...
module ClickSend
  class RequestAlphaTagRequest < ApiModelBase
    attr_accessor :alpha_tag
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClickSend::RequestAlphaTagRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClickSend::RequestAlphaTagRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'alpha_tag')
        self.alpha_tag = attributes[:'alpha_tag']
      end
      ...
      if attributes.key?(:'businesses')
        if (value = attributes[:'businesses']).is_a?(Array)
    ...
    def list_invalid_properties
      ...
      if !@countries.nil? && @countries.length > 1
        invalid_properties.push('invalid value for "countries", number of items must be less than or equal to 1.')
      end
      ...
      if !@businesses.nil? && @businesses.length > 1
        invalid_properties.push('invalid value for "businesses", number of items must be less than or equal to 1.')
      end

      if !@businesses.nil? && @businesses.length < 1
        invalid_properties.push('invalid value for "businesses", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@countries.nil? && @countries.length < 1
      return false if !@businesses.nil? && @businesses.length > 1
      return false if !@businesses.nil? && @businesses.length < 1
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] businesses Value to be assigned
    def businesses=(businesses)
      if businesses.nil?
        fail ArgumentError, 'businesses cannot be nil'
      end

      if businesses.length > 1
        fail ArgumentError, 'invalid value for "businesses", number of items must be less than or equal to 1.'
      end

      if businesses.length < 1
        fail ArgumentError, 'invalid value for "businesses", number of items must be greater than or equal to 1.'
      end
    ...
    def hash
      [alpha_tag, reason, countries, businesses].hash
    end
