  ...
  class SendEmailCampaignData < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClickSend::SendEmailCampaignData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClickSend::SendEmailCampaignData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'totlal_count')
        self.totlal_count = attributes[:'totlal_count']
      end
      ...
      if attributes.key?(:'queued_count')
        self.queued_count = attributes[:'queued_count']
      end
    ...
    def hash
      [totlal_count, total_price, queued_count, data, _currency].hash
    end
