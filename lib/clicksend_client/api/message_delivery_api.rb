...
module ClickSend
  class MessageDeliveryApi
    ...
    def create_delivery_issue(opts = {})
      data, _status_code, _headers = create_delivery_issue_with_http_info(opts)
      data
    end
    ...
    # @option opts [String] :content_type 
    # @option opts [CreateDeliveryIssueRequest] :create_delivery_issue_request 
    # @return [Array<(CreateDeliveryIssue, Integer, Hash)>] CreateDeliveryIssue data, response status code and response headers
    def create_delivery_issue_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: MessageDeliveryApi.create_delivery_issue ...'
      end
      # resource path
      local_var_path = '/v3/delivery-issues'
      ...
      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(opts[:'create_delivery_issue_request'])

      # return_type
      return_type = opts[:debug_return_type] || 'CreateDeliveryIssue'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['basicAuth']

      new_options = opts.merge(
        :operation => :"MessageDeliveryApi.create_delivery_issue",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: MessageDeliveryApi#create_delivery_issue\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
    ...
    def get_all_delivery_issues(opts = {})
      data, _status_code, _headers = get_all_delivery_issues_with_http_info(opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [String] :content_type 
    # @return [Array<(GetAllDeliveryIssues, Integer, Hash)>] GetAllDeliveryIssues data, response status code and response headers
    def get_all_delivery_issues_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: MessageDeliveryApi.get_all_delivery_issues ...'
      end
      # resource path
      local_var_path = '/v3/delivery-issues'
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'GetAllDeliveryIssues'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['basicAuth']

      new_options = opts.merge(
        :operation => :"MessageDeliveryApi.get_all_delivery_issues",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: MessageDeliveryApi#get_all_delivery_issues\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
