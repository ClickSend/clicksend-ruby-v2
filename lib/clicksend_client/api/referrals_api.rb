...
module ClickSend
  class ReferralsApi
    ...
    def view_referral_accounts(opts = {})
      data, _status_code, _headers = view_referral_accounts_with_http_info(opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [String] :content_type 
    # @return [Array<(ViewReferralAccounts, Integer, Hash)>] ViewReferralAccounts data, response status code and response headers
    def view_referral_accounts_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ReferralsApi.view_referral_accounts ...'
      end
      # resource path
      local_var_path = '/v3/referral/accounts/'
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'ViewReferralAccounts'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['basicAuth']

      new_options = opts.merge(
        :operation => :"ReferralsApi.view_referral_accounts",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ReferralsApi#view_referral_accounts\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
