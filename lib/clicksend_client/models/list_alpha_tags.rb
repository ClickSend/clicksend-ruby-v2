...
module ClickSend
  class ListAlphaTags < ApiModelBase
    # The list of alpha tags.
    attr_accessor :alpha_tags
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClickSend::ListAlphaTags` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClickSend::ListAlphaTags`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'alpha_tags')
        if (value = attributes[:'alpha_tags']).is_a?(Array)
          self.alpha_tags = value
        end
      end
