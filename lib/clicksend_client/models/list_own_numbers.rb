...
module ClickSend
  class ListOwnNumbers < ApiModelBase
    # The list of own numbers.
    attr_accessor :own_numbers
    ...
    def self.openapi_types
      {
        :'own_numbers' => :'Array<OwnNumber>',
        :'_metadata' => :'ListOwnNumbersMetadata'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ClickSend::ListOwnNumbers` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ClickSend::ListOwnNumbers`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'own_numbers')
        if (value = attributes[:'own_numbers']).is_a?(Array)
          self.own_numbers = value
        end
      end
